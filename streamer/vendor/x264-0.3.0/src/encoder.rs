...
// Added: force-IDR variant
impl Encoder {
    /// Like `encode` but forces this frame to be encoded as an IDR keyframe.
    pub fn encode_idr(&mut self, pts: i64, image: Image) -> Result<(Data, Picture)> {
        assert_eq!(image.width(), self.width());
        assert_eq!(image.height(), self.height());
        assert_eq!(image.encoding(), self.encoding());
        unsafe {
